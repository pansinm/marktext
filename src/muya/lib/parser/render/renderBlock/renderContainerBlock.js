...
const PRE_BLOCK_HASH = {
  ...
  graphviz: `.${CLASS_OR_ID.AG_GRAPHVIZ}`,
...
export default function renderContainerBlock (parent, block, activeBlocks, matches, useCache = false) {
  ...
  if (/^(?:th|td)$/.test(type)) {
    ...
    if (
      /html|multiplemath|flowchart|mermaid|sequence|vega-lite|graphviz/.test(functionType)
    ) {
    ...
    if (/html|multiplemath|mermaid|flowchart|vega-lite|sequence|graphviz/.test(functionType)) {
